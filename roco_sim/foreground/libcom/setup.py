...
def get_ext_modules(cur_dir):
    ...
    def cuda_paths():
        cuda_home = os.environ.get("CUDA_HOME") or TORCH_CUDA_HOME or "/usr/local/cuda"
        cand_includes = [
            os.path.join(cuda_home, "targets", "x86_64-linux", "include"),
            os.path.join(cuda_home, "include"),
        ]
        include_dir = next((p for p in cand_includes if os.path.exists(os.path.join(p, "cuda_runtime.h"))), None)
        lib64_dir = os.path.join(cuda_home, "lib64")
        return cuda_home, include_dir, lib64_dir

    if torch.cuda.is_available():
        cuda_home, cuda_include, cuda_lib64 = cuda_paths()
        if cuda_include is None:
            raise RuntimeError(
                "Could not find cuda_runtime.h. "
                "Set CUDA_HOME to your CUDA toolkit (e.g. /usr/local/cuda-12.9)."
            )

        if torch.__version__ >= '1.11.0':
            trilinear_cpp = join(cur_dir, 'libcom/image_harmonization/source/trilinear_cpp_torch1.11/src/trilinear_cuda.cpp')
            trilinear_cu  = join(cur_dir, 'libcom/image_harmonization/source/trilinear_cpp_torch1.11/src/trilinear_kernel.cu')
        else:
            trilinear_cpp = join(cur_dir, 'libcom/image_harmonization/source/trilinear_cpp/src/trilinear_cuda.cpp')
            trilinear_cu  = join(cur_dir, 'libcom/image_harmonization/source/trilinear_cpp/src/trilinear_kernel.cu')

        return CUDAExtension(
            name='trilinear',
            sources=[trilinear_cpp, trilinear_cu],
            include_dirs=[cuda_include],
            library_dirs=[cuda_lib64],
            extra_compile_args={
                'cxx': ['-O3'],
                'nvcc': ['-Xcompiler', '-fPIC']  # 与上面命令行保持一致
            }
        )
    else:
